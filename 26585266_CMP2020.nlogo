...
to go ; observer
  ...
  ask snakes [
    if god-mode [ ;; Creative spin; rainbow snake world builder
      set snake-color snake-color + 1
      if snake-color = green [ set snake-color snake-color + 1 ]
    ]
    ...
    ifelse god-mode[
      ask patches with [pcolor < (([snake-color] of myself) - ([snake-age] of myself)) and (pcolor != grey) and (pcolor != green) ] [ ;; edited to allow god mod to run smoother.
        set age age + 1
    ...
    [
      ...
      ask patches with [cost != -1] [ ;; set patches cost based on the distance to the apple (yum)
        let goal one-of patches with [pcolor = green]
        set cost distance goal
      ]
    ]
...
to-report DFS
  ...
  let goal one-of patches with [pcolor = green]
  let pat (list )
  ask start [ set parent nobody ]
  let visited_list (list )
  ;if start = goal[report pat]
  let frontiers (list start) ;; all the different paths it can take to apple
  loop[
    if empty? frontiers [show "Failed" report (list )] ;; if theres nothing you can do = death
    let check last frontiers
    set frontiers remove-item (length frontiers - 1) frontiers ; gets rid of the path youre looking at so its not reused
    set visited_list lput check visited_list
    ;ask check [set visited true] ;; lets the check know where the snake is
    ;ask check [set pcolor 60] ;; debug
    foreach [vnp] of check [vnps ->
      if not member? vnps visited_list and not member? vnps frontiers[ask vnps [set parent check]
        if vnps = goal [
          set visited_list (list )
          report plan-recovery goal]
        set frontiers lput vnps frontiers
      ]
    ]
  ]
end
...
to-report BFS
  ...
  loop[
    if empty? frontiers [show "Failed" report (list )] ;; if theres nothing you can do = death
    let check first frontiers
    set frontiers remove-item (0) frontiers ; gets rid of the path youre looking at so its not reused
    set visited_list fput check visited_list
    ;ask check [set visited true] ;; lets the check know where the snake is
    ;ask check [set pcolor 60] ;; debug
    foreach [vnp] of check [vnps ->
      if not member? vnps visited_list and not member? vnps frontiers[ask vnps [set parent check]
...
to-report UCS
  ...
  let pat (list)
  ask start [ set parent nobody ]
  let visited-list (list)
  if start = goal [ report pat ]
  let frontiers (list start)
  ;; used bubble sort to sort cost in uniform crosspath search
  foreach frontiers [ fr ->
    foreach frontiers [ fro ->
      if fro != fr [
        if [cost] of fr < [cost] of fro [
          let temp fr
          set fr fro
          set fro fr
        ]
  ...
  loop[
    ;; very similar code to BFS and DFS
    if empty? frontiers [ show "Failed" report (List)]
    let check first frontiers
    set frontiers remove-item (position check frontiers) frontiers
    set visited-list fput check visited-list
    foreach [vnp] of check [ vnps ->
      ;; sets cost to length of path
      ask vnps [ set cost (length (plan-recovery vnps)) ]
        if not member? vnps visited-list and not member? vnps frontiers[ask vnps [set parent check]
        if vnps = goal [
          set visited-list (list )
          report plan-recovery goal]
        set frontiers lput vnps frontiers
        ;; resorts the list when new item is added
        foreach frontiers [ fr ->
          foreach frontiers [ fro ->
            if fro != fr [
              if [cost] of fr < [cost] of fro [
...
to-report GBFS
  ...
  loop[
    ;; very similar code to BFS and DFS
    if empty? frontiers [ show "Failed" report (List)]
    let check first frontiers ;; finish
    set frontiers remove-item (position check frontiers) frontiers
    set visited-list fput check visited-list
    foreach [vnp] of check [ vnps ->
      ;; sets cost to distance to apple
      ask vnps [ set cost (distance goal) ]
      if not member? vnps visited-list and not member? vnps frontiers[ask vnps [set parent check]
        if vnps = goal [
          set visited-list (list )
          report plan-recovery goal]
...
to-report ASS
  ...
  loop[
    ...
    set visited-list fput check visited-list
    foreach [vnp] of check [ vnps ->
      ;; sets cost to the distance to apple and the length of the list
      if-else vnps = 0 [
        ask vnps [ set cost ((distance goal)) ]
      ] [ask vnps [ set cost ((distance goal) + length (plan-recovery vnps)) ]]
      if not member? vnps visited-list and not member? vnps frontiers[ask vnps [set parent check]
        if vnps = goal [
          set visited-list (list )
          report plan-recovery goal]
...
;;---------------------
to-report vnp ;; this is valid next patches
  let dirs []
  if member? [pcolor] of patch-at 0 1 clear-colors [set dirs lput patch-at 0 1 dirs]
  if member? [pcolor] of patch-at 1 0 clear-colors [set dirs lput patch-at 1 0 dirs]
  if member? [pcolor] of patch-at 0 -1 clear-colors [set dirs lput patch-at 0 -1 dirs]
  if member? [pcolor] of patch-at -1 0 clear-colors [set dirs lput patch-at -1 0 dirs]
  report dirs
end
...
to-report plan-recovery [node] ;; recovery plan for patches
  let plan (list node)
  if [parent] of node = nobody [report (list node)]
  report rpr [parent] of node plan
end

to-report rpr [node plan]
  set plan fput node plan ;; add node to the list plan
...
red-team-mode
red-team-mode
"human" "random" "DFS" "BFS" "UCS" "ASS" "GBFS"
5
...
blue-team-mode
blue-team-mode
"human" "random" "DFS" "BFS" "UCS" "GBFS" "ASS" "GBFS"
6
...
Student Number: 26585266

## Extensions made

Made a creative extension known as "God Mode" a rainbow snake world builder switch.
The snake changes colours purposefully cycling through green and grey to place apples and wall blocks onto the screen to create the "God" mode effect.
...
Harman, H.(2024) Adversarial Search [Workshop]. Artiifical Intelligence CMP2020, University Of Lincoln, Avaliable from: https://blackboard.lincoln.ac.uk/ultra/courses/_189065_1/cl/outline [accessed 12 March 2024]
